...
const Create = ({ idReq, clientes, servicos }) => {
  ...
  const handleChangeService = (e, index) => {
    ...
    services[index].name = servicos[e.currentTarget.value].Nome;
    services[index].descricao =
      servicos[parseInt(e.currentTarget.value)].Descrição;
    services[index].id = "";
  ...
  const subTotal = (index, desconto_type) => {
    if (services[index].name.includes("HH")) {
      ...
      if (services[index].service !== "") {
        ...
        const service_preco = servicos[parseInt(service_id)].Preço;
